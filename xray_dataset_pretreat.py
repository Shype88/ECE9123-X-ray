...
def main():
    ...
    #                 transforms.ToTensor(),
    # ])
    # db = torchvision.datasets.ImageFolder(root='face_test', transform=tf)
    ...
    db_train = xray('face_test', 64, 'val')
    db_test = xray('face_test', 64, 'test')

    x,y = next(iter(db_train))
    print('sample:', x.shape, y.shape, y)

    viz.image(db_train.denormalize(x), win='sample_x', opts=dict(title='sample_x'))

    loader = DataLoader(db_train, batch_size=32, shuffle=True, num_workers=8)
